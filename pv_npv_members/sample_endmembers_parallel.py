...
with ProcessPoolExecutor(max_workers=8) as executor:
    ...
    for i, future in enumerate(as_completed(futures)):
        ...
        with lock:
          ...
          if len(all_pv):
            pd.concat(list(all_pv), ignore_index=True).to_pickle('pv_spectra_debug.pkl')
          if len(all_npv):
            pd.concat(list(all_npv), ignore_index=True).to_pickle('npv_spectra_debug.pkl')
          print(f'---Saved samples from file {i+1}/{len(unique_files)}---')
...
# Change from lnf codes to text
pv_crop['crop_type'] = pv_crop['crop_type'].map(lnf_mapping)
npv_crop['crop_type'] = npv_crop['crop_type'].map(lnf_mapping)
