...
def load_data(parcel_shp, s2_data_dir, yr, fc_dir, soil_group, chunk_size=10):
    ...
    preds = preds.drop_vars('product_uri')
...
def plot_timeseries_clean(df_parcel, mean_PV, mean_NPV, mean_Soil, dates_clean, mean_PV_clean, mean_NPV_clean, mean_Soil_clean, save_path):
    ...
    # Plot vertical dashed lines for each activity
    df_parcel = df_parcel.dropna(subset=['datum'])
    df_parcel = df_parcel[(df_parcel['datum'] >= dates_clean[0]) & (df_parcel['datum'] <= dates_clean[-1])]  # Filter activities within clean dates

    # Remove activities on the same date
    df_parcel = df_parcel.drop_duplicates(subset=['datum'], keep='last')
    ...
    for _, row in df_parcel.iterrows():
        ...
        # Mark the activity and its date
        ax.axvline(x=row['datum'], color=color, linestyle='--', alpha=0.7)
        if row['activity'] == 'Sowing intermediate crop':
            ax.text(row['datum']-pd.Timedelta(days=1), 1, row['activity'], rotation=45, verticalalignment='bottom', fontsize=14)
        else:
            ax.text(row['datum']-pd.Timedelta(days=3), 1, row['activity'], rotation=45, verticalalignment='bottom', fontsize=14)



    # Lineplot for clean dates
    if len(dates_clean):
        ax.plot(dates_clean, mean_PV_clean.values, label='Green Vegetation', color='g', linewidth=2)
        ax.plot(dates_clean, mean_NPV_clean.values, label='Brown Vegetation', color='mediumslateblue', linewidth=2, alpha=0.7)
        ax.plot(dates_clean, mean_Soil_clean.values, label='Soil', color='saddlebrown', linewidth=2)

    # Set plot formatting
    ax.set_ylim(0, 1) 
    plt.xlabel('Date', fontsize=16)
    plt.ylabel('Mean Fractional Cover [-]', fontsize=16)
    #plt.title('Predicted soil cover')
    ax.set_xlim(dates_clean[0]-pd.Timedelta(days=5), dates_clean[-1]+pd.Timedelta(days=10))
    ...
    handles.append(residue_line)
    labels.append("Remove residues")
    ax.legend(loc='upper left', bbox_to_anchor=(0.82, 1.5), fontsize=14)
...
def plot_images_clean_seperate(df_parcel, ds, dates_clean, preds, parcel_shp, buffer_plot, save_folder, suffix):
    # Ensure the save folder exists
    os.makedirs(save_folder, exist_ok=True)

    # Shorten S2 timeseries to field calendar
    start_date = dates_clean[0]  # Start at the first clean date
    end_date = dates_clean[-1]  # End at the last clean date
    ds = ds.sel(time=slice(start_date, end_date))
    preds = preds.sel(time=slice(start_date, end_date))
    ...
    for i, date in enumerate(dates_clean):
        ...
        x_coords = rgb[i].coords['lon'].values
        y_coords = rgb[i].coords['lat'].values
        ax.imshow(rgb[i, :, :] * brightness, origin='upper', extent=[x_coords.min(), x_coords.max(), y_coords.min(), y_coords.max()])
        ax.set_title(f"{str(rgb.time.values[i]).split('T')[0]}", fontsize=30)
        ax.axis('off')

        # Add outline of field
        parcel_shp.plot(ax=ax, alpha=0.5, edgecolor='r', facecolor='None', linewidth=4)

        # Save the image
        save_path = os.path.join(save_folder, f"RGB_{str(date).split('T')[0]}_{suffix}.png")
...
def anim_s2_clean(df_parcel, parcel_shp, buffer_plot, ds, preds, dates_clean, suffix, save_path):
    ...
    # Initialize the time series plot
    time_series_ax = fig.add_subplot(gs[1, :])
    time_series_ax.spines['top'].set_visible(False)
    time_series_ax.spines['right'].set_visible(False)
    ...
    time_series_ax.set_ylim(0, 1)
    time_series_ax.set_ylabel("Mean field cover", fontsize=18)
    time_series_ax.set_xlim(dates_clean[0]-pd.Timedelta(days=5), dates_clean[-1]+pd.Timedelta(days=10))
    ...
    # Compute the mean PV, NPV, and soil fractions for the field and new time range
    dates_clean = np.array([d for d in preds.time.values if d in dates_clean])
    mean_PV = preds[f'PV_norm_{suffix}'].mean(dim=['lon', 'lat']).sel(time=dates_clean)
    mean_NPV = preds[f'NPV_norm_{suffix}'].mean(dim=['lon', 'lat']).sel(time=dates_clean)
    mean_Soil = preds[f'Soil_norm_{suffix}'].mean(dim=['lon', 'lat']).sel(time=dates_clean)
    ...
    # Plot vertical dashed lines for each activity
    df_parcel = df_parcel.dropna(subset=['datum']).drop_duplicates(subset='datum', keep='last')
    df_parcel = df_parcel[(df_parcel['datum']>=dates_clean[0]) & (df_parcel['datum']<=dates_clean[-1])]

    for _, row in df_parcel.iterrows():
        remove_residues = row['ernteresteeingearbeitet']
        color = 'red' if remove_residues else 'gray'
        time_series_ax.axvline(x=row['datum'], color=color, linestyle='--', alpha=0.7)
        time_series_ax.text(row['datum'], 1, row['activity'], rotation=45, verticalalignment='bottom', fontsize=10)
    ...
    def update(frame):
        ...
        # Create a custom for "residues" line
        handles, labels = time_series_ax.get_legend_handles_labels()
        time_series_ax.legend(handles[:3], ['PV','NPV', 'Soil'], loc='upper left', bbox_to_anchor=(1.1, 1))
...
# Results storage
results_dir = os.path.expanduser('~/mnt/eo-nas1/eoa-share/projects/028_Erosion/Erosion/spectral_unmixing/results/ZA-AUI/NPV_check')


########################
# DEFINE FIELD TO LOOK AT

yr = 2021
crop = 'Wintergerste'
betrID = 50023000105
parcel_name = 'FontainedeBerneuvésin'



########################
# GENERATE MP4 ANIMATION FOR FIELD
...
if parcel_shp.is_empty.any() or parcel_shp.geometry.area.any()<150:
    ...
    parcel_shp = farms_polys[(farms_polys.betr_ID==betrID) & (farms_polys.farmname==farm_name) & (farms_polys.name==parcel_name)].to_crs(32632)          
    #parcel_shp.to_file('FCreport_geom.gpkg')
...
acitivity_dict = json.load(open(os.path.expanduser('~/mnt/eo-nas1/eoa-share/projects/028_Erosion/Erosion/spectral_unmixing/code/activities_dict.json'), 'r'))
...
#########################
# LOAD FC DATA  
ds, preds, soil_group_parcel = load_data(parcel_shp, s2_data_dir, yr, fc_dir, soil_group, chunk_size=10)
...
if ds is not None and soil_group_parcel is not None: 
    ...
    #save_path = 'test.mp4'
    #anim_s2_clean(df_parcel, parcel_shp, buffer_plot, ds, preds, dates_clean, suffix, save_path)

    save_path = 'FCreport_timseries_2021_wintergerste_FontainedeBerneuvesin.png'
    plot_timeseries_clean(df_parcel, mean_PV, mean_NPV, mean_Soil, dates_clean, mean_PV_clean, mean_NPV_clean, mean_Soil_clean, save_path)

    save_folder = 'FCreport_imgs'
    #plot_images_clean_seperate(df_parcel, ds, dates_clean, preds, parcel_shp, buffer_plot, save_folder, suffix)