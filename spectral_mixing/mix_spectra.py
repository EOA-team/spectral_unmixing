...
pv_file = os.path.expanduser('~/mnt/eo-nas1/eoa-share/projects/010_CropCovEO/Erosion/pv_npv_members/summarised_pv_samples.pkl')
npv_file = os.path.expanduser('~/mnt/eo-nas1/eoa-share/projects/010_CropCovEO/Erosion/pv_npv_members/summarised_npv_samples.pkl')
...
for feature in range(classes):
  ...
  for i in range(iterations):
    ...
    features_df = pd.DataFrame(features_list, columns=['B2', 'B3', 'B4', 'B8', 'B11', 'B12'])
...
pv_endmembers = pv_endmembers[S2_cols].rename(columns=dict(zip(S2_cols, clean_names)))[input_bands]
npv_endmembers = npv_endmembers[S2_cols].rename(columns=dict(zip(S2_cols, clean_names)))[input_bands]
#soil_endmembers = soil_endmembers[SRC_cols].rename(columns=dict(zip(SRC_cols, clean_names)))
shadow_endmember =  np.full(len(input_bands), 0.01)
...
iterations = 5 # Number of tiems to create a dataset
classes = 3 # (1=NPV, 2=PV, 3=Soil)
num_mixtures = 1000  # Number of synthetic mixtures per class
