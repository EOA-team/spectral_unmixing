...
if ds is not None and soil_group_parcel is not None: 
    ...
    if soil_group == 0:
        save_path = os.path.join(results_dir, f'fulltime/{parcel_name.replace(" ", "").replace("/", "")}_FC_timeseries_imgs_gapfill.png')
    else:
        save_path = os.path.join(results_dir, f'fulltime/{parcel_name.replace(" ", "").replace("/", "")}_FC_timeseries_imgs_soil{soil_group_parcel}_gapfill.png')
    print(save_path)
    plot_timeseries_images_gapfill(df_parcel, ds, dates_clean, dates_raw, preds, parcel_shp, buffer_plot, save_path, gapfilling_list)