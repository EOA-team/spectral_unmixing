...
def plot_timeseries_images_gapfill(df_parcel, ds, dates_clean, dates_raw, preds, parcel_shp, buffer_plot, save_path, gapfilling):
    ...
    if len(dates_clean):
        ...
        algo = gapfilling.get('algorithm')
        kwargs = gapfilling.get('kwargs', {})

        if any(key in algo.__name__ for key in ('loess', 'spline', 'savgol', 'gpr')):
            result = algo(
    ...
    # Lineplot for clean dates
    if len(dates_clean):
        ax_ts.scatter(dates_clean, mean_PV_clean.values, label='Cleaned PV', color='g', marker='s', s=20)
        ax_ts.scatter(dates_clean, mean_NPV_clean.values, label='Cleaned NPV', color='goldenrod', marker='s', s=20)
        ax_ts.scatter(dates_clean, mean_Soil_clean.values, label='Cleaned Soil', color='saddlebrown', marker='s', s=20)

    # Plot gapfilled
    ax_ts.plot(dates_smooth, pv_smooth, color='g', label='Gapfilled PV', linewidth=2)
    ax_ts.plot(dates_smooth, npv_smooth, color='goldenrod', label='Gapfilled NPV', linewidth=2)
    ax_ts.plot(dates_smooth, soil_smooth, color='saddlebrown', label='Gapfilled Soil', linewidth=2)

    # Add uncertainty bands (shaded area between 5th and 95th percentile)
    ax_ts.fill_between(dates_smooth, pv_q05, pv_q95, color='g', alpha=0.2)
    ax_ts.fill_between(dates_smooth, npv_q05, npv_q95, color='goldenrod', alpha=0.2)
    ax_ts.fill_between(dates_smooth, soil_q05, soil_q95, color='saddlebrown', alpha=0.2)
    ...
    ax_ts.set_xlabel('Date')
    ax_ts.set_ylabel('Fraction')
    ax_ts.set_title('Predicted soil cover (Gapfilled)')
    ax_ts.set_xlim(df_parcel['datum'].min()-pd.Timedelta(days=20), df_parcel['datum'].max()+pd.Timedelta(days=20))
...
def remove_spikes(data, max_days=10, threshold=0.1):
    ...
    for i in range(1, len(values)-1):
        ...
        # Spike up: current rises sharply from previous and drops after
        rel_up = (cur_val - prev_val) / prev_val
        drop_after = (cur_val - next_val) / cur_val if cur_val != 0 else 0

        # Spike down: current drops sharply from previous and rises after
        rel_down = (prev_val - cur_val) / prev_val
        rise_after = (next_val - cur_val) / cur_val if cur_val != 0 else 0

        if dt_next <= max_days/2:
            if rel_up > threshold and drop_after > threshold:
                removed_points.append((times[i], cur_val))
                values[i] = np.nan
            elif rel_down > threshold and rise_after > threshold:
                removed_points.append((times[i], cur_val))
                values[i] = np.nan

    cleaned_data = xr.DataArray(values, coords=data.coords, dims=data.dims, name=data.name)
    return cleaned_data, removed_points


def zscore_spikes(data, max_days=10, z_thresh=3):
    """
    Remove spikes in a time series using z-score and reversion check.

    Parameters:
    - data: xarray.DataArray with 'time' dimension
    - max_days: max time window to consider a spike
    - z_thresh: z-score threshold to consider an outlier
    
    Returns:
    - cleaned_data: DataArray with spikes removed
    - removed_points: list of (timestamp, original_value)
    """
    ...
    rolling_mean = data.rolling(time=max_days, center=True).mean()
    rolling_std = data.rolling(time=max_days, center=True).std()
    rolling_z = (data - rolling_mean) / rolling_std

    for i in range(1, len(values) - 1):
        z = rolling_z[i].values
        if np.isnan(z) or abs(z) <= z_thresh:
            continue

        dt_next = (times[i + 1] - times[i]).astype('timedelta64[D]').astype(int)
        z_next = rolling_z[i + 1].values

        if dt_next <= max_days / 2 and np.abs(z_next) < z_thresh:
            removed_points.append((times[i], values[i]))
            values[i] = np.nan
...
def gpr(dates, pv_vals, npv_vals, soil_vals, pred_dates=None, kernel=None, alpha=1e-6):
    """
    ...
    pv_mean, pv_std = fit_predict(pv_vals)
    npv_mean, npv_std = fit_predict(npv_vals)
    soil_mean, soil_std = fit_predict(soil_vals)
...
if ds is not None and soil_group_parcel is not None: 
    ...
    ####################################################################################################
    # TIMESERIES SPIKE DETECTION (NO INTERPOLATION)

    """
    smoothing_dict = {
        'algorithm': remove_spikes,
        'kwargs': {
            'max_days': 10,
            'threshold': 0.2
        }
    ...
    if soil_group == 0:
        save_path = os.path.join(results_dir, f'{parcel_name.replace(" ", "").replace("/", "")}_FC_timeseries_imgs_cloudcl.png')
    else:
        save_path = os.path.join(results_dir, f'{parcel_name.replace(" ", "").replace("/", "")}_FC_timeseries_imgs_soil{soil_group_parcel}.png')
    print(save_path)
    plot_timeseries_images(df_parcel, ds, dates_clean, dates_raw, preds, parcel_shp, buffer_plot, save_path, smoothing_dict)
    """
    ...
    """
    gapfill_dict = {
        'algorithm': loess_bootstrap_normalized,
        'kwargs': {
            'frac': 0.1,
            'n_bootstrap': 1
        }
    ...
    gapfill_dict = {
        'algorithm': kalman_with_obs_uncertainty
    }
    ...
    # PLOT TIMESERIES W/ IMAGES - STATIC
    if soil_group == 0:
        save_path = os.path.join(results_dir, f'za-aui/{parcel_name.replace(" ", "").replace("/", "")}_FC_timeseries_imgs_gpr.png')
    else:
        save_path = os.path.join(results_dir, f'za-aui/{parcel_name.replace(" ", "").replace("/", "")}_FC_timeseries_imgs_soil{soil_group_parcel}.png')
    print(save_path)
    plot_timeseries_images_gapfill(df_parcel, ds, dates_clean, dates_raw, preds, parcel_shp, buffer_plot, save_path, gapfill_dict)